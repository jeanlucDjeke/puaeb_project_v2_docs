...
for(i = 0; i < sbi.geo.conf.measures.length; i++) {
		console.log("Start Measure logs");
		console.log("Nom mesure" + sbi.geo.conf.measures[i].name);
		console.log("End Measure logs");
}*/
...
ThematicWindow.prototype.addThemes = function() {
      ...
      //first a few styles
			...
			var isParamYear = 0;// Paramètre pour savoir si i a excédé l'index du paramètre année
    	for(i = 0; i < sbi.geo.conf.measures.length; i++) {
			
			if(sbi.geo.conf.measures[i].name !='annee_ins_ad'){
				if(isParamYear==1)
					offset = 50 + (30*(i-1));
				else
					offset = 50 + (30*i);
			...
			= new checkBox(
              sbi.geo.conf.measures[i].name,"radioButtons",30,offset, "radioBorder", "radioPoint",false,sbi.geo.conf.measures[i].description,labeltextStyles,12,radioGroup,undefined);
				...
				isParamYear = 1;//Lorsque l'index i passe par le parametre annee isParamYear passe à 1
			}
		}
